...
def rmse(img_array: np.ndarray, compressed_img_array: np.ndarray) -> float:
    return np.sqrt(np.mean((img_array-compressed_img_array)**2))
...
if __name__ == "__main__":    
    ...
    if "load" not in st.session_state or not st.session_state.initialized:
        st.session_state["load"] = cv2.VideoCapture(0)
        st.session_state["frame"] = st.empty()   
        st.session_state["slider"] = st.empty()
        st.session_state["button"] = st.empty()
        st.session_state.initialized = True
        
    # Display the webcam with SVD compression
    if "init" not in st.session_state:
        st.session_state["button"] = button_placeholder.button("Stop video", on_click=destroy)
        st.session_state["slider"] = slider_placeholder.slider("Number of components:", 
                                min_value=1, max_value=100, value=5) 
        
        while st.session_state["load"].isOpened() and not st.session_state["button"]:
            ret, frame = st.session_state["load"].read()
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY).astype(np.float32)
            
            u, s, v = np.linalg.svd(gray, full_matrices=False)

            compressed_img_array = u[:, :st.session_state["slider"]] @ np.diag(s[:st.session_state["slider"]]) @ v[:st.session_state["slider"], :]
            
            st.session_state["frame"].image((compressed_img_array/255).astype(np.float32), 
                                    channels="GRAY", clamp=True, use_column_width=True)

            if cv2.waitKey(1) & 0xFF == ord('q') or st.session_state["button"]:
                st.session_state["load"].release()
                cv2.destroyAllWindows()
                break    
    ...
    if uploaded_file is not None:
        ...
        n = st.slider("Number of components:", min_value=1, 
                        max_value=max(img_array.shape[:2])//3, value=10, key="slider_img")
        ...
        if gray_checkbox:
            ...
            print(compressed_img_array.shape)
        else:
            # RGB image
        ...
        st.subheader(f"Compression rate: {(100*compression_rate(n, img_array.shape[0], img_array.shape[0])):.2f}%")    
